...
function getIntro(){
	...
	echo "		<h3 align='center'>$r[nama_ujian]</h3><br/>
		Waktu Pengerjaan : $r[waktu] menit<br/>
		Jumlah Soal : $hitung<br/><br/>
		<p/>
		<h2>PERATURAN</h2><br/>
		$r[peraturan]<br/>
		";
...
function countcategories($category_id){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $categoriescount => $categorydata) {
			...
			$sql="SELECT * FROM tbl_data WHERE subkategori_id='$categoryid'";
			if ($result=mysqli_query($con,$sql)) {
					# code...
...
function countcategoriesmain(){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $categoriescount => $categorydata) {
				#count how many times each category appears in blogs
			$categoryid=$categorydata['id'];
			$sql="SELECT * FROM tbl_data WHERE kategori_id='$categoryid'";
			if ($result=mysqli_query($con,$sql)) {
					# code...
				$rowcountcategory=mysqli_num_rows($result);
				$getcatcount=$rowcountcategory;
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $sliderposts => $slideritem) {
      	# code...fetch actual category from categories table
			...
			echo '<li>
			<div class="blog-item">
			<img src="../foto/'.$slideritem['gambar'].'" alt="fantastic cms" class="img-fluid" style="width:100%;height:auto"/>
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $bloggrid => $griditem) {
      	# code...
			...
			<li>
			<a href="#">
			<i class="far fa-clock"></i>'.$griditem['tanggal'].'</a>
			</li>
			
			<li>
			<a href="#">
			<i class="fas fa-tags"></i>'.$griditem['tag'].'</a>
			</li>
...
function getcategoryblogs($table,$id){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE kategori_id='$id' ORDER BY id DESC LIMIT 10";
	if ($result=mysqli_query($con,$sql))
	{
      	//count number of rows in query result
		$rowcount=mysqli_num_rows($result);
			...
			echo '<div class="col-md-6 card">
							...
							<div class="card-body">
								<ul class="blog-icons my-4">
									<li>
										<a href="#">
											<i class="far fa-calendar-alt"></i> '.$cdata['tanggal'].'</a>
									</li>
									<li class="mx-2">
										<a href="#">
											<i class="far fa-user"></i> '.$cdata['judul'].'</a>
									</li>
									<li>
										<a href="#">
											<i class="fas fa-tags"></i> '.$cdata['tag'].'</a>
									</li>

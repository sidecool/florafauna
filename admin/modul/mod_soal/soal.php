...
switch($_GET[act]){
  ...
  echo "<div style='text-align:right;padding-right:30px;'>
    ...
    echo"<datalist id='auto'>";
     $qry=mysqli_query($connect,"SELECT * FROM tbl_soal");
  ...
  //Tampil Data Soal    
     echo" <table class='table table-striped table-bordered table-hover'>
          <tr><th>No</th><th>Pertanyaan</th><th>Aksi</th><th>Status</th></tr>"; 
    $tampil=mysqli_query($connect,"SELECT * FROM tbl_soal ORDER BY id_soal DESC");
    $no=1;
    ...
    $soal=substr($r[soal],0,78);
       echo "<tr><td>$no</td>
             <td>$soal..</td>
             <td>
        <a href=?module=soal&act=editsoal&id=$r[id_soal] class='btn  btn btn-primary'><i class='fa fa-pencil-square-o'></i></a> 
        
            <a href='?module=soal&act=viewsoal&id=$r[id_soal]' class='btn btn-primary'\"><i class='fa fa-eye'></i></a>
            <a href=$aksi?module=soal&act=hapus&id=$r[id_soal] class='btn  btn btn-danger'><i class='fa fa-trash'></i></a>
  ...
  // Form Edit Soal  
  case "editsoal":
    $edit=mysqli_query($connect,"SELECT * FROM tbl_soal WHERE id_soal='$_GET[id]'");
  ...
  case "viewsoal":
    $view=mysqli_query($connect,"SELECT * FROM tbl_soal WHERE id_soal='$_GET[id]'");
  ...
  case "carisoal":
    ...
    $tampil=mysqli_query($connect,"SELECT * FROM tbl_soal WHERE soal LIKE '%$_POST[cari]%'");
    $no=1;
    while ($r=mysqli_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             <td>$r[soal]</td>
